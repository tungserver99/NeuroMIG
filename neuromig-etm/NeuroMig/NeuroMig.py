...
class NeuroMig(nn.Module):
    def __init__(self, vocab_size, num_topics=50, num_groups=10, en_units=200, dropout=0.,
                 pretrained_WE=None, plm_emb_size=384, embed_size=200, beta_temp=0.2,
        ...
        # for InfoNCE
        self.prj_rep = nn.Sequential(nn.Linear(self.num_topics, plm_emb_size),
                                     nn.Dropout(dropout))
...
#     def compute_loss_TM(self, x, recon_x, mu, logvar, avg_loss=True):
...
#         loss_TM = self.compute_loss_TM(bow, recon_x, mu, logvar, avg_loss)

#         #loss_ECR = self.get_loss_ECR()
#         loss_InfoNCE = self.compute_loss_InfoNCE(theta, contextual_emb)
#         if epoch_id == 10 and self.group_connection_regularizer is None:
#             self.create_group_connection_regularizer()
#         if self.group_connection_regularizer is not None and epoch_id > 10:
#             loss_GR = self.get_loss_GR()
