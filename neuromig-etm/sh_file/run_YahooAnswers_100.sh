...
for weight_GR in "${weight_GR_values[@]}"; do
    for weight_InfoNCE in "${weight_InfoNCE_values[@]}"; do
        ...
        python main.py --wandb_prj NeuroMig_Yahoo_100K --model NeuroMig --dataset YahooAnswers --num_topics 100 --beta_temp 0.2 --num_groups 20 --epochs 500 --device cuda --lr 0.002 --lr_scheduler StepLR --dropout 0 --batch_size 200 --lr_step_size 125 --use_pretrainWE --weight_ECR 40 --weight_GR "$weight_GR" --alpha_ECR 20.0 --alpha_GR 5.0 --weight_InfoNCE "$weight_InfoNCE"
    done
done
